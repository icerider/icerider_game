...
minetest.register_tool("tech_tweak:pick_carbon_steel", {
...
minetest.register_tool("tech_tweak:pick_stainless_steel", {
...
minetest.register_craft({
	output = 'tech_tweak:pick_carbon_steel',
	recipe = {
...
minetest.register_craft({
	output = 'tech_tweak:pick_stainless_steel',
	recipe = {
...
local function quarry_area(pos, node)
	...
	local startpos = vector.add(vector.add(vector.add(pos,
		vector.new(0, 0, 0)),
		pdir),
		vector.multiply(qdir, -50))
	local endpos = vector.add(vector.add(vector.add(startpos,
		vector.new(0, -99, 0)),
		vector.multiply(pdir, 100)),
		vector.multiply(qdir, 100))
...
--						meta:from_table()	-- this is undocumented (!)
		...
		if row >= 100 then meta:set_int("enabled", 0) end
	end -- if enabled
...
minetest.register_node("tech_tweak:quarry", {
...
technic.register_machine("HV", "tech_tweak:quarry", technic.receiver)

--------------------------------------------------------------------------------------
-- stone ladder

minetest.register_node("tech_tweak:ladder", {
...
minetest.register_node("tech_tweak:cobble", {
...
if minetest.get_modpath("mesecons_mvps") ~= nil then
	mesecon:register_mvps_stopper("tech_tweak:cobble")
end

technic.register_compressor_recipe(
	{ input = { "default:cobble 99" },
	output = "tech_tweak:cobble" }
)

minetest.register_craft({
	output = "default:cobble 99",
	type = "shapeless",
	recipe = {"tech_tweak:cobble"}
})

--------------------------------------------------------------------------------------
-- clean glass that connects textures (if enabled)

minetest.register_node("tech_tweak:glass", {
...
minetest.register_node("tech_tweak:light", {
...
minetest.register_craftitem("tech_tweak:silver_coil", {
	description = "Silver Coil",
	inventory_image = "techtweak_silver_coil.png",
})

minetest.register_craft({
	output = 'tech_tweak:silver_coil',
	recipe = {
...
minetest.register_craft({
	...
	recipe = {
		{'technic:carbon_steel_ingot', 'tech_tweak:silver_coil', 'technic:carbon_steel_ingot'},
		{'technic:carbon_steel_ingot', 'tech_tweak:silver_coil', 'technic:carbon_steel_ingot'},
...
minetest.register_craft({
	...
	recipe = {
		...
		{'tech_tweak:silver_coil', 'moreores:tin_ingot', 'tech_tweak:silver_coil'},
		{'moreores:tin_ingot',      'moreores:tin_ingot', 'moreores:tin_ingot'},
	}
})
...
minetest.register_craft({
	...
	recipe = {
		...
		{'tech_tweak:silver_coil', 'technic:carbon_steel_ingot', 'tech_tweak:silver_coil'},
		{'technic:carbon_steel_ingot', 'technic:carbon_steel_ingot', 'technic:carbon_steel_ingot'},
	}
})
...
minetest.register_craft({
	...
	recipe = {
		...
		{'tech_tweak:silver_coil', 'technic:stainless_steel_ingot', 'tech_tweak:silver_coil'},
		{'technic:stainless_steel_ingot', 'technic:stainless_steel_ingot', 'technic:stainless_steel_ingot'},
	}
})
...
minetest.register_node("tech_tweak:supply_converter", {
...
minetest.register_craft({
	output = 'tech_tweak:supply_converter',
	recipe = {
...
for tier, machines in pairs(technic.machines) do
	technic.register_machine(tier, "tech_tweak:supply_converter", technic.producer_receiver)
end
...
minetest.register_craft({
	output = 'tech_tweak:hv_centrifuge',
	recipe = {
...
local cen_run = function(pos, node)
	...
	if minetest.get_node(pos).name == "tech_tweak:hv_centrifuge_active" then
		is_on = true
	end
	...
	if not eu_input then
		...
		if eu_input >= demand then
			local dtime = 5
			if mode == 2 then dtime = 4 end
			for i = 1, 5 do
				local result = technic.get_recipe(rtype, { inv:get_stack("src", i) })
				if result then
					activate = true

					-- everything takes 5 seconds, right?
					if stime[i] >= dtime then
						...
						for _, o in ipairs(output) do
							...
							if not inv:room_for_item("dst", s) then
								-- overflow literally overflows
								minetest.item_drop(s, "", {x=pos.x, y=pos.y+1, z=pos.z})
							else
								inv:add_item("dst", s)
							end
						end
	...
	if activate and not is_on then
		technic.swap_node(pos, "tech_tweak:hv_centrifuge_active")
	elseif not activate and is_on then
		technic.swap_node(pos, "tech_tweak:hv_centrifuge")
	end
end

minetest.register_node("tech_tweak:hv_centrifuge", {
...
minetest.register_node("tech_tweak:hv_centrifuge_active", {
		description = "HV Centrifuge Array",
		tiles = { "technic_mv_centrifuge_top.png", "technic_mv_centrifuge_bottom.png",
			"technic_mv_centrifuge_side.png", "technic_mv_centrifuge_side.png",
			"technic_mv_centrifuge_side.png", "techtweak_centrifuge_front_active.png" },
		paramtype2 = "facedir",
		drop = "tech_tweak:hv_centrifuge",
		groups = {cracky=2, technic_machine=1, technic_hv=1,
...
technic.register_machine("HV", "tech_tweak:hv_centrifuge", technic.receiver)
technic.register_machine("HV", "tech_tweak:hv_centrifuge_active", technic.receiver)
...
local fur_run = function(pos, node)
	...
	local is_on = false
	local activate = false
	if minetest.get_node(pos).name == "tech_tweak:mv_furnace_active" then
		is_on = true
	end
	...
	if activate and not is_on then
		technic.swap_node(pos, "tech_tweak:mv_furnace_active")
	elseif not activate and is_on then
		technic.swap_node(pos, "tech_tweak:mv_furnace")
	end

end

minetest.register_node("tech_tweak:mv_furnace", {
...
minetest.register_node("tech_tweak:mv_furnace_active", {
		description = "MV Advanced Furnace",
		tiles = { "technic_mv_electric_furnace_top.png", "technic_mv_electric_furnace_bottom.png",
			"technic_mv_electric_furnace_side.png", "technic_mv_electric_furnace_side.png",
			"technic_mv_electric_furnace_side.png", "techtweak_furnace_front_active.png" },
		paramtype2 = "facedir",
		drop = "tech_tweak:mv_furnace",
...
technic.register_machine("MV", "tech_tweak:mv_furnace", technic.receiver)
technic.register_machine("MV", "tech_tweak:mv_furnace_active", technic.receiver)
...
minetest.register_craft({
	output = 'tech_tweak:mv_geothermal',
	recipe = {
...
minetest.register_craft( {
	output = "tech_tweak:hv_geothermal",
	recipe = {
		{'tech_tweak:mv_geothermal', 'tech_tweak:mv_geothermal', 'tech_tweak:mv_geothermal'},
		{'tech_tweak:mv_geothermal', 'technic:hv_transformer', 'tech_tweak:mv_geothermal'},
		{'',                   'technic:hv_cable', ''},
	},
})
...
local geo_run = function(pos, node)
	...
	local tier_name        = "MV"
	local name_off         = "tech_tweak:mv_geothermal"
	local name_on          = "tech_tweak:mv_geothermal_active"
	...
	if n == "tech_tweak:hv_geothermal" or n == "tech_tweak:hv_geothermal_active" then
		supply_name = "HV_EU_supply"
		tier_name = "HV"
		name_off = "tech_tweak:hv_geothermal"
		name_on = "tech_tweak:hv_geothermal_active"
		unit_value = 1875	-- 75 * 5 * 5 (7500 at full power)
	end

	-- check whether it really is active
	if n == "tech_tweak:mv_geothermal_active" or n == "tech_tweak:hv_geothermal_active" then
		is_on = true
	end
...
minetest.register_node("tech_tweak:mv_geothermal", {
...
minetest.register_node("tech_tweak:mv_geothermal_active", {
	description = "Geothermal MV Array",
	...
	legacy_facedir_simple = true,
	sounds = default.node_sound_wood_defaults(),
	drop = "tech_tweak:mv_geothermal",
	technic_run = geo_run,
})

minetest.register_node("tech_tweak:hv_geothermal", {
...
minetest.register_node("tech_tweak:hv_geothermal_active", {
	description = "Geothermal HV Array",
	...
	legacy_facedir_simple = true,
	sounds = default.node_sound_wood_defaults(),
	drop = "tech_tweak:hv_geothermal",
	technic_run = geo_run,
})

technic.register_machine("MV", "tech_tweak:mv_geothermal",        technic.producer)
technic.register_machine("MV", "tech_tweak:mv_geothermal_active", technic.producer)
technic.register_machine("HV", "tech_tweak:hv_geothermal",        technic.producer)
technic.register_machine("HV", "tech_tweak:hv_geothermal_active", technic.producer)
