...
function PlayerState_ops:initialize(newState)
   ...
   if newState then
      ...
      self.state.speed_mod = self.state.speed_mod or 1.0;
      self.state.jump_mod = self.state.jump_mod or 1.0;
...
function PlayerState_ops:updatePhysics(es, fs, hs, ps)
   ...
   local sm = calcMultiplier(settings.speedMultipliers, es, fs, hs, ps) * self.state.speed_mod * self.state.speed_armor;
   local jm = calcMultiplier(settings.jumpMultipliers, es, fs, hs, ps) * self.state.jump_mod * self.state.jump_armor;
